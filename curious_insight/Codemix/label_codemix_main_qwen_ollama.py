#!/usr/bin/env python3
"""Label whether codemix main answers are hallucinated using Ollama qwen2.5:32b.

Reads a CSV with at least: id, codemix_context, codemix_question, codemix_answer.
Writes a new CSV with an added column `is_hallucinated`.

Usage:
    python Codemix/label_codemix_main_qwen_ollama.py \
        --input "Codemix/Main dataset/codemix_1000.csv" \
        --output "Results/codemix_1000_main_qwen_labeled.csv" \
...
def main() -> None:
    ...
    p.add_argument("--model", default="qwen2.5:32b", help="Ollama model name")
    p.add_argument(
        "--ollama-url",
        default=None,
    ...
    p.add_argument("--answer-column", default="codemix_answer", help="Answer column name")
    args = p.parse_args()
    ...
    with out.open("a", newline="", encoding="utf-8") as ofh:
        ...
        for i, r in enumerate(selected, start=args.start):
            ...
            answer = r.get(args.answer_column, "")
